<?php
	...
	//define('DB_HOST', 'localhost');	//Your hosting. 99% you don't need to change this
    //define('DB_USER', 'root');			//Your MySQL username
	...
	if($result) {
		if(mysql_num_rows($result) == 1) {
			...
			if(isset($_SERVER['HTTP_REFERER'])) {
				...
				header("location:kursus/berjadual/keterangan/".$id."");
			}
			else {
				header("location:kursus/luar_jadual/keterangan/".$id."");
			}
			exit();
		} 
		else {
			//Login failed
			$this->session->set_flashdata('error','Sila semak no kad pengenalan/kata laluan anda');
			header("location:access-denied");
			exit();
		}
	} 
